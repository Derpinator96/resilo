...
// ─────────────────────────────────────────────────────────────
// Power Factor Arc (semi-circle, 0-1)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Upgraded High-Precision Metric Counter Card (Cohesive Light Theme)
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
const API_BASE     = import.meta.env.VITE_API_URL || "https://unicef-solar-energy-project-v2.onrender.com";
const LATEST_URL   = `${API_BASE}/api/telemetry/latest`;
const HISTORY_URL  = `${API_BASE}/api/telemetry/history?limit=60`;
...
export default function IoTMonitor() {
  ...
  const fetchLatest = useCallback(async () => {
    ...
    try {
      ...
      const json = await res.json();
      const newData = parsePayload(json.data ?? json);
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 md:p-6">
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        {/* HEADER */}
        <div className="bg-[#011425] rounded-xl p-6 flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-3xl font-bold text-white">IoT Live Monitor</h1>
            <p className="text-white/50 mt-1 text-sm">Frontend Integrated Time-Series Energy Audit Console</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-sm font-semibold ${connected ? "text-emerald-400" : "text-red-400"}`}>
              {connected ? "Pipeline Syncing" : "Data Disconnected"}
            </span>
            {lastUpdated && (
              <span className="text-white/30 text-xs ml-2">Updated {lastUpdated.toLocaleTimeString()}</span>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm px-5 py-4 border border-slate-100 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            {connected && <span className="text-emerald-600 text-xs font-medium">✓ System Active</span>}
