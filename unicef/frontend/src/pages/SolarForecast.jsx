...
export default function SolarForecastPage() {
    ...
    const districts = Object.keys(chhattisgarhDistrictsHealthCentres);
    const centres   = district ? chhattisgarhDistrictsHealthCentres[district] : [];
    ...
    const fetchForecast = async () => {
        ...
        try {
            const { data } = await axios.get(`${BACKEND}/api/ml/forecast`, {
                params: { district, centreName: centre },
            ...
            setEqLoading(true);
            try {
                const eqResp = await axios.get(`${BACKEND}/api/ml/equipment`, {
                    params: { district, centreName: centre },
                    withCredentials: true
                });
                setEquipment(eqResp.data.data);
            } catch (e) {
            ...
            setError(e.response?.data?.message || "Failed to fetch forecast");
        } finally {
            setLoading(false);
        }
    };
                                                ...
                                                <div>
                                                    ...
                                                    <p className={`text-sm mt-1 ${
                                                        ...
                                                        {energyResult.sufficient
                                                            ...
                                                            : `${energyResult.surplus} kWh shortfall for ${energyResult.deliveries} delivery${energyResult.deliveries > 1 ? "s" : ""}. Grid backup or load scheduling recommended.`
                                                        }
                                                    </p>
                                                </div>
                                            </div>
